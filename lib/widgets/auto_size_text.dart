...
class AutoSizeGroupBuilder extends StatefulWidget {
  ...
  @override
  createState() => _AutoSizeGroupBuilderState();
}
...
class _AutoSizeTextState extends State<AutoSizeText> {
  ...
  /// Builds the text widget with the given font size, style, and max lines.
  Widget _buildText(double fontSize, TextStyle style, int? maxLines) {
    if (widget.data != null) {
      return Text(
        ...
        textScaler: widget.textScaler ?? const TextScaler.linear(1),
        maxLines: maxLines,
  ...
  /// Calculates the appropriate font size for the text to fit within the given constraints.
  List _calculateFontSize(BoxConstraints size, TextStyle? style, int? maxLines) {
    ...
    final userScaler = widget.textScaler ?? const TextScaler.linear(1);
    ...
    if (presetFontSizes == null) {
      ...
      final defaultScale = userScaler.scale(defaultFontSize) / style.fontSize!;
      if (_checkTextFits(span, defaultScale, maxLines, size)) {
        return <Object>[userScaler.scale(defaultFontSize), true];
      }
    ...
    while (left <= right) {
      ...
      double scale;
      if (presetFontSizes == null) {
        scale = mid * userScaler.scale(widget.stepGranularity) / style!.fontSize!;
      } else {
    ...
    double fontSize;
    if (presetFontSizes == null) {
      fontSize = right * userScaler.scale(widget.stepGranularity);
    } else {
  ...
  /// Checks if the text fits within the given constraints at the specified scale.
  ...
  /// Notifies the widget to synchronize its state.
  void _notifySync() {
    setState(() {});
  }

  /// Validates the properties of the widget.
